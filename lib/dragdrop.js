...
Dragdrop = (function(superClass) {
  ...
  Dragdrop.prototype._bind = function() {
    return this.el.on('mousedown.simple-dragdrop', this.opts.draggable, (function(_this) {
      return function(e) {
        var $target, handleEl, pos;
        if ($.isFunction(_this.opts.except) && _this.opts.except(e)) {
          return;
        }
        e.stopPropagation();
        if (_this.dragging) {
        ...
        handleEl = $target.find(_this.opts.handle).get(0);
        if (_this.opts.handle && !$.contains(handleEl, e.target) && handleEl !== e.target) {
          return;
        }
        pos = {
          top: e.pageY,
  ...
  Dragdrop.prototype._dragStart = function($target, pos) {
    ...
    $body = $('body').css('cursor', this.opts.cursor);
    this._disableSelection($body);
    this._renderHelper();
    this._initPosition(pos);
    this._renderPlaceholder();
    if (this.triggerHandler('dragstart', {
  ...
  Dragdrop.prototype._dragEnd = function() {
    ...
    this.el.off('mouseenter.simple-dragdrop mousemove.simple-dragdrop');
    $body = $('body').css('cursor', '');
    return this._enableSelection($body);
  };

  Dragdrop.prototype._disableSelection = function($el) {
    return $el.css({
      'user-select': 'none'
    });
  };

  Dragdrop.prototype._enableSelection = function($el) {
    return $el.css({
      'user-select': ''
    });
  };
